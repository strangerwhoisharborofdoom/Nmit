      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Review incoming time-off requests, track historical leaves, and manage company PTO quotas.
          </p>
        </div>
      ...
      {activeTab === 'pending' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">Requires Manager / HR Decision</span>
          </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {pendingLeaves.map((req) => (
                    <tr key={req.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-4 px-6 font-semibold text-slate-800">{req.leaveTypeName}</td>
                      ...
                      <td className="py-4 px-6 text-slate-600 max-w-xs truncate">{req.remarks || '—'}</td>
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs transition-colors flex items-center gap-1"
                          >
                          ...
                          <button
                            ...
                            disabled={isProcessing}
                            className="px-3.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl font-bold text-xs transition-colors flex items-center gap-1"
                          >
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="py-3.5 px-6">Reviewer / Notes</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
            ...
            {leaveTypes.map((type) => (
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span
                      className={cn(
                        ...
                        type.isPaid ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-700'
                      )}
                    >
                  ...
                  <p className="text-xs text-slate-400 mb-3">{type.description || 'Standard policy leave'}</p>

                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-xs">
                    <div className="flex justify-between font-semibold">
                      <span className="text-slate-500">Annual Quota:</span>
                      <span className="text-slate-900 font-bold">{type.defaultDays} Days</span>
                    </div>
                  </div>
                </div>
